...
# ── Функции ──
def _use_mock_ml() -> bool:
    flag = os.getenv("ML_USE_MOCK", "").strip().lower()
    logger.info(f"ML_USE_MOCK env var: '{flag}'")
    if flag in ("1", "true", "yes", "on"):
        logger.info("✅ Используем mock-режим по переменной ML_USE_MOCK")
        return True
    key = (YANDEX_API_KEY or "").strip()
    folder = (YANDEX_FOLDER_ID or "").strip()
    logger.info(f"YANDEX_API_KEY: '{key[:10]}...' if key else 'empty'")
    logger.info(f"YANDEX_FOLDER_ID: '{folder[:10]}...' if folder else 'empty'")
    if not key or not folder:
        logger.info("✅ Используем mock-режим (пустые API credentials)")
        return True
    if key.startswith("your-") or folder.startswith("your-"):
        logger.info("✅ Используем mock-режим (placeholder credentials)")
        return True
    logger.info("❌ Используем реальный Yandex GPT API")
    return False


def _mock_subtasks(task_title: str) -> list[str]:
    title = (task_title or "задача").strip()
    return [
        f"Подготовиться: {title}",
...
async def generate_subtasks(request: SubtaskGenerationRequest):
    ...
    try:
        ...
        if _use_mock_ml():
            logger.info("Используем mock-генерацию (ML_USE_MOCK или не задан Yandex API)")
            subtasks = _mock_subtasks(request.task_title)
        else:
